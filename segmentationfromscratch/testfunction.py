...
def test(test_loader, model, directory_name, test_csv):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            # # Save NIfTI file
            # pred_seg_np = pred_seg.argmax(dim=1)[0].cpu().numpy().squeeze().astype(np.float32)  # Save as class indices
            ...
            # nii_img = nib.Nifti1Image(pred_seg_np, affine)
    ...
    # print(f"Average Test Loss: {avg_loss:.4f}, Avg Dice: {avg_dice:.4f}")
    # wandb.log({"Average Test Loss": avg_loss, "Average Dice": avg_dice})
